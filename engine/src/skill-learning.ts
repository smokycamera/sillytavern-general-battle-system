...
export function learnAbilities(unit: Combatant, specs: BlueprintSpec[], opts: { replace?: boolean; prepared?: string[]; rebuildRequested?: boolean } = {}): Combatant {
  ...
  for (const spec of specs) {
    ...
    if (!skillDefinitionKnown(id)) throw new Error('未知学习机制');
    ...
    if (candidates.length > 1 || instanceId && !candidates.length) throw new Error('技能身份不唯一或已失效，请在面板选择具体实例');
    ...
    if (requested !== undefined && (!Number.isSafeInteger(requested) || requested < 1 || requested > 10)) throw new Error('技能规格必须为1–10整数');
  ...
  if (new Set(next.abilities.map((a) => a.id)).size !== next.abilities.length) throw new Error('技能实例身份冲突');
  if (opts.prepared) {
