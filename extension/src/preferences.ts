...
export interface DisplayPreferences { theme?: 'dark' | 'light'; geometry?: { left: number; top: number; width: number; height: number } }
export function preferences(host: HostWindow): { read(): DisplayPreferences; write(value: DisplayPreferences): void } {
  return {
    read() {
      ...
      const value = raw as DisplayPreferences; const geometry = value.geometry;
      return { theme: value.theme === 'light' ? 'light' : 'dark', ...(geometry && Object.values(geometry).every(Number.isFinite) && geometry.width >= 320 && geometry.height >= 260 ? { geometry: { ...geometry } } : {}) };
    },
    write(value) {
