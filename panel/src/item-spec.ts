import { ACCESSORY_NAMES, CONSUMABLE_NAMES } from '../../engine/src/items.js';
...
export function itemSpecificationLabel(spec: ItemSpecification): string {
  const name = spec.kind === 'weapon' ? WEAPON_CLASSES[spec.mechanism]?.name ?? spec.mechanism
    : spec.kind === 'armor' ? ['无甲', '轻甲', '中甲', '重甲', '超重甲'][spec.tier] : spec.kind === 'shield' ? '盾牌' : spec.kind === 'accessory' ? ACCESSORY_NAMES[spec.mechanism] : CONSUMABLE_NAMES[spec.mechanism];
...
export function parseItemSpecification(text: string, attrs: { type?: string; quality?: string; body?: string; enchant?: string; stabilized?: string; protection?: string } = {}): ItemSpecification {
  ...
  if (weapon) {
    if (attrs.protection !== undefined) throw new Error('防护类型只适用于护甲');
    if (attrs.enchant !== undefined && !['none', 'thermal', 'arcane'].includes(attrs.enchant)) throw new Error('未知附魔效果');
    ...
    else if (accessory) spec = { kind: 'accessory', mechanism: accessory as keyof typeof ACCESSORY_NAMES, ...base };
    else if (consumable) spec = { kind: 'consumable', mechanism: consumable as keyof typeof CONSUMABLE_NAMES, ...base };
    else if (['治疗', 'heal'].includes(name)) spec = { kind: 'consumable', mechanism: 'heal', ...base };
    else throw new Error('未知物品用途，不能按陌生名字推断能力');
  }
  if (attrs.protection !== undefined && spec.kind !== 'armor') throw new Error('防护类型只适用于护甲');
  const type = spec.kind === 'shield' ? 'armor' : spec.kind;
  if (attrs.type !== undefined && attrs.type !== type) throw new Error('物品种类与物品属性冲突');
  return spec;
}
