...
export function renderReportWorkspace(b: Battle | null, reports: BattleReport[], selectedId: string | undefined, deliveries: BattleDeliveries, renderLog: (b: Battle) => string,
  ...
  return `<section class="report-workspace"><h2>${over?'战报与叙述':'当前战况'}</h2><p class="sub">战报独立保存，在发送前可读档。“完整逐轮战报”发送整场记录，推荐使用。可在本轮对话结束后手动修改为[战报]节约token。超长战斗可用叙述战斗终章，只发送战前战后双方单位状态及伤害来源</p>${!b?`<label>历史战报<select data-role="report-select">${reports.map((r,i)=>`<option value="${esc(r.id)}" ${r.id===report?.id?'selected':''}>${esc(title(r,i))}</option>`).join('')}</select></label>`:''}
