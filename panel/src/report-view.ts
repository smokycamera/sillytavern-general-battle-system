...
export function renderReportWorkspace(b: Battle | null, reports: BattleReport[], selectedId: string | undefined, deliveries: BattleDeliveries, renderLog: (b: Battle) => string,
  ...
  return `<section class="report-workspace"><h2>${over?'战报与叙述':'当前战况'}</h2><p class="sub">战报独立保存，收兵后仍可发送。原生模式点击战报只添加用户消息，再点击“发送给 AI”或酒馆发送按钮才生成回复。“发送新增战况”接着上一次用此按钮确认发送的位置继续；首次使用从本场开头发送。“完整逐轮战报”会重新发送整场记录。</p>${!b?`<label>历史战报<select data-role="report-select">${reports.map((r,i)=>`<option value="${esc(r.id)}" ${r.id===report?.id?'selected':''}>${esc(title(r,i))}</option>`).join('')}</select></label>`:''}
    ...
    <div class="report-send-actions">${over?`<button class="primary" data-action="out-epilogue" ${finished?'disabled':''}>${finished?'终章已发送':'叙述战斗终章'}</button>`:''}<button data-action="out-digest">发送完整逐轮战报</button><button class="${over?'':'primary'}" data-action="out-delta" ${!delta||blocked?'disabled':''}>发送新增战况</button></div>
