...
export function unitForm(prefix: string, d: UnitDraft, registry: Map<string, Trait>, opts: { editing?: boolean; managed?: boolean } = {}): string {
  ...
  return `<div data-builder-form="${prefix}" class="unit-builder"><div class="builder-basics">
    ...
    <div class="builder-recommendation"><span>当前方案</span><b>${esc(d.primary.name || WEAPON_CLASSES[d.primary.mechanism]?.name || '主武器')} · ${['无甲', '轻甲', '中甲', '重甲', '超重甲'][Number(d.armor.tier)] ?? '护甲'}${d.mount ? ' · 骑乘' : ''}${d.shield ? ' · 携盾' : ''}</b><small>${opts.editing ? '修改后先预览，未改动的实例保持原样。' : '默认无甲、L1主武器；需要装备或特殊能力时再展开调整。'}</small></div>
    ...
    <details class="builder-section" data-detail-id="${prefix}-skills"><summary>技能 <span>${d.skills.length ? d.skills.length + '项已学' : '按需要添加'}</span></summary>
      ...
      ${d.skills.map((s, i) => `<div class="builder-skill"><input data-role="${prefix}-skill" data-index="${i}" list="${prefix}-mechanisms" value="${esc(skillDefinitionName(s.id))}" aria-label="通用技能机制" placeholder="物理单体、魔法范围、buff、debuff"><input data-role="${prefix}-skill-name" data-index="${i}" value="${esc(s.name)}" placeholder="自定名字（可省略）"><input data-role="${prefix}-skill-power" data-index="${i}" type="number" min="1" max="10" value="${esc(s.power)}" aria-label="技能规格" ${ABILITY_BLUEPRINTS[s.id]?.fixedPower ? 'readonly title="固定预备来源"' : ''}>
      ...
      <datalist id="${prefix}-mechanisms">${[...SKILL_CATEGORIES.map((c) => c.name), '物理单体近战', '物理单体射击', '物理范围击退', '魔法范围热能+燃烧', 'buff范围治疗', 'buff加速', 'buff净化', 'buff回能', 'buff召唤', 'buff特质飞行', 'debuff定身', 'debuff眩晕', 'debuff沉默', 'debuff驱散', 'debuff拉拽'].map((name) => '<option value="' + esc(name) + '"></option>').join('')}</datalist>
      <button data-action="builder-skill-add" data-builder="${prefix}">添加技能</button>${d.skills.some((s) => s.id === 'bp-call-reinforce') ? `<label>预备份额${input('reserves', 'type="number" min="0" max="2"')}</label>` : ''}
      <p class="sub">填写自定义名称、通用机制与等级。六类为物理单体、物理范围、魔法单体、魔法范围、buff、debuff；需要具体效果时在机制后加治疗、加速、定身等。最多准备${MAX_PREPARED_SKILLS}项，同机制共享冷却。旧实例名称仅用于保留原效果。</p>
    </details>
...
export function buildPreview(unit: Combatant, before?: Combatant): string {
  ...
  return `<div class="builder-preview" data-role="builder-preview"><h3>${before ? '确认档案变化' : '确认加入队伍'}</h3><b>${esc(unit.name)}</b><p>${change(`${unit.hp}/${unit.base.hpMax}`, before && `${before.hp}/${before.base.hpMax}`)} ${unit.scale === 'hero' ? '生命' : '人数'} · 训练${unit.level}${unit.scale!=='hero'?' · 成员耐久 '+memberDurability(unit):''}</p><p>${change(gear(unit), before && gear(before))}</p>
    ...
    <p>${esc(anchoredWeaponLabel(unit.weapon))}</p><p>负载 ${load}/${BODY[unit.body ?? 'human'].capacity} · 防护 动能${anchoredProtection(unit, 'kinetic')} / 热能${anchoredProtection(unit, 'thermal')} / 奥术${anchoredProtection(unit, 'arcane')} · 装甲等效耐久×${Number(armorPowerScale(unit).toFixed(2))}</p>
