...
当前发布版本为 **0.2.0-rc.13**，内置 JEV **0.2.3**。原有自动 AI 保留为默认；新安装与旧存档不会自动切换到 JEV。
...
API Key、地址、协议和模型 ID 都保存在本机浏览器存储中，刷新、重开面板或更新扩展后继续可用，不写入聊天存档。同源扩展可以访问浏览器存储，因此只应在可信客户端保存密钥。已有旧桥接配置不会被自动覆盖；切换官方直连时请同时修改协议、地址和密钥。
...
结束我方回合后，先保存已经完成的操作，再等待 JEV；每个自动单位完成行动后分别保存。暂停、重新读取或切换控制器不会撤回此前已经确认的行动。敌方回合恢复与玩家操作共用执行队列，等待期间收到新的存档通知后会接续最新战场。下一次轮到我方单位时，移动和主行动额度由引擎正常恢复。

本版同时重新生成实际安装的 `dist`，包含此前的 TauriTavern 宿主连接、普通 LLM JSON 兼容和 API Key 持久保存修复。开发者提交前运行 `npm run build:extension` 和 `npm run verify:dist`，CI 会检查安装文件是否对应当前源码。
...
保存失败沿用既有 journal 回滚／核实流程，重试不重新请求模型或掷骰。刷新恢复已保存配置与计划，全自动不会自行打开。密钥不进入 dist、计划、战报或聊天存档；API Key／服务令牌保存在本机浏览器存储中，便于刷新、重开面板和扩展更新后继续使用。同源扩展可访问该存储。不自动向聊天发送 JEV 决策。
